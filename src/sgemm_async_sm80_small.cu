...
static_assert(SGBLAS_SM80_SMALL_TILE_COLUMNS > 0 &&
                  SGBLAS_SM80_SMALL_TILE_COLUMNS <= 1024,
              "SM80 small tile columns must be in [1, 1024]");
static_assert(SGBLAS_SM80_SMALL_TILE_COLUMNS % 16 == 0,
              "SM80 small tile columns must be a multiple of 16");
static_assert(SGBLAS_SM80_SMALL_THREAD_ROWS > 0 &&
                  SGBLAS_SM80_SMALL_THREAD_ROWS <= 64,
              "SM80 small thread rows must be in [1, 64]");
static_assert(SGBLAS_SM80_SMALL_MIN_BLOCKS_PER_SM > 0 &&
                  SGBLAS_SM80_SMALL_MIN_BLOCKS_PER_SM <= 32,
              "SM80 small launch-bounds residency must be in [1, 32]");
static_assert(SGBLAS_SM80_SMALL_MAX_WIDE_CTAS >= 0);
static_assert(SGBLAS_SM80_SMALL_SECOND_MIN_WIDE_CTAS >= 0);
static_assert(SGBLAS_SM80_SMALL_SECOND_MAX_WIDE_CTAS >=
              SGBLAS_SM80_SMALL_SECOND_MIN_WIDE_CTAS);
static_assert(SGBLAS_SM80_SMALL_SECOND_MAX_M >= 0);
static_assert(SGBLAS_SM80_SMALL_SECOND_MAX_N >= 0);
static_assert(SGBLAS_SM80_SMALL_MIN_K >= 0);

constexpr int kTileRows = 64;
constexpr int kTileColumns = SGBLAS_SM80_SMALL_TILE_COLUMNS;
constexpr int kTileK = 32;
constexpr int kThreads = 128;
constexpr int kThreadRows = SGBLAS_SM80_SMALL_THREAD_ROWS;
...
constexpr int kBStride = kTileK;
constexpr int kBTileElements = kTileColumns * kTileK;
constexpr int kBTileStorageElements = kTileColumns * kBStride;
constexpr int kSharedBytes =
    2 * (kATileElements + kBTileStorageElements) * sizeof(float);
...
#pragma unroll
  for (int vector_load = 0; vector_load < b_vector_loads_per_thread;
    ...
    copyAsync16(b_tile + stage * kBTileStorageElements +
                    local_column * kBStride + local_k,
                source);
  }
  commitAsyncCopies();
}
...
__global__ __launch_bounds__(kThreads, SGBLAS_SM80_SMALL_MIN_BLOCKS_PER_SM) void
sgemmAsyncNnSmallKernel(
    int m, int n, int k, float alpha, const float *a, int lda, const float *b,
    int ldb, float beta, float *c, int ldc) {
  extern __shared__ __align__(16) float shared[];
  float *a_tile = shared;
  float *b_tile = shared + 2 * kATileElements;
...
#pragma unroll
      for (int output_column = 0; output_column < kOutputsColumns;
        ...
        b_values[output_column] =
            b_tile[stage * kBTileStorageElements +
                   (thread_column + output_column * kThreadColumns) *
                       kBStride +
                   reduction];
      }
#pragma unroll
...
sgblasStatus_t launchKernel(dim3 grid, dim3 block, int m, int n, int k,
  ...
  prepareLaunchStatus();
  if (cudaFuncSetAttribute(sgemmAsyncNnSmallKernel<Mode, NMajorRaster>,
  ...
  sgemmAsyncNnSmallKernel<Mode, NMajorRaster>
...
sgblasStatus_t launchAsyncNnSm80Small(
    int m, int n, int k, float alpha, const float *a, int lda, const float *b,
